...
final class CompressionMethodTests: XCTestCase {
    ...
    func testGzipRoundTrip() throws {
        ...
        XCTAssertNotEqual(originalData, compressedData, "Dados comprimidos devem ser diferentes dos originais")
        
        // Descomprime e verifica se volta ao original
        let decompressedData = try decompressData(compressedData, method: .gzip)
        XCTAssertEqual(originalData, decompressedData, "Dados descomprimidos devem ser iguais aos originais")
    }
    ...
    func testLZFSERoundTrip() throws {
        ...
        let decompressedData = try decompressData(compressedData, method: .lzfse)
        XCTAssertEqual(originalData, decompressedData)
    }
    
    func testLZ4RoundTrip() throws {
        let originalData = "Teste para LZ4, dados de exemplo".data(using: .utf8)!
