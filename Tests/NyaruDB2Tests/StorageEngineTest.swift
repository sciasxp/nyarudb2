...
final class StorageEngineTests: XCTestCase {
    ...
    func testInsertAndFetchWithoutPartition() async throws {
        ...
        let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("StorageEngineTest")
        try? FileManager.default.removeItem(at: tempURL)
        let storage = StorageEngine(path: tempURL.path, shardKey: nil, compressionMethod: .none)
    ...
    func testInsertAndFetchWithPartition() async throws {
        ...
        let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("StorageEngineTestPartition")
        try? FileManager.default.removeItem(at: tempURL)
        let storage = StorageEngine(path: tempURL.path, shardKey: "category", compressionMethod: .lzfse)
