...
final class StatsEngineTests: XCTestCase {
    ...
    func testGetCollectionStats() async throws {
        ...
        XCTAssertEqual(
            stats.numberOfShards,
            1,
            "Se não há particionamento, espera-se apenas o shard 'default'"
        )
        XCTAssertEqual(stats.totalDocuments, 3, "A contagem total deve ser 3")
        XCTAssertTrue(
            stats.totalSizeInBytes > 0,
            "O tamanho total deve ser maior que zero"
        )
        XCTAssertEqual(
            stats.shardDetails.count,
            1,
            "Deve haver 1 detalhe (1 shard)"
        )
    }
    ...
    func testGetGlobalStats() async throws {
        ...
        XCTAssertEqual(
            ...
            "O total de documentos deve ser a soma"
        )
        XCTAssertTrue(
            globalStats.totalSizeInBytes > 0,
            "O tamanho global deve ser maior que zero"
        )
    }
}
