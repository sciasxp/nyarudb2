...
public actor StorageEngine {
    ...
    public init(path: String, shardKey: String? = nil, compressionMethod: CompressionMethod = .none) {
    ...
    public func insertDocument<T: Codable>(_ document: T, collection: String) async throws {
        // Se o particionamento estiver ativo, extrai o valor do campo correspondente.
        var shardID: String
        if let key = shardKey {
            let jsonData = try JSONEncoder().encode(document)
            guard let dict = try JSONSerialization.jsonObject(with: jsonData, options: []) as? [String: Any],
                  let shardValue = dict[key] as? String else {
                throw NSError(domain: "StorageEngine", code: 1, userInfo: [NSLocalizedDescriptionKey: "Shard key '\(key)' não encontrado no documento"])
            }
            shardID = shardValue
        } else {
        ...
        if shardManager == nil {
            ...
            shardManager = ShardManager(baseURL: collectionURL, compressionMethod: compressionMethod)
